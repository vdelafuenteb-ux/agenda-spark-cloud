...
import { Search, X, User, ChevronsDownUp, ChevronsUpDown, Mail, CalendarClock, ChevronDown, ArrowUpDown, Check, Tag, Building2 } from 'lucide-react';
...
export function FilterBar({ searchQuery, onSearchChange, allTags, selectedTagIds, onToggleTag, assignees, selectedAssignee, onAssigneeChange, departments, selectedDepartment, onDepartmentChange, forceExpand, onToggleExpand, onBulkEmail, filterNoDueDate, onToggleNoDueDate, showOngoing = true, showNotOngoing = true, onToggleShowOngoing, onToggleShowNotOngoing, sortBy = 'order', onSortChange }: FilterBarProps) {
  const hasDateFilter = onToggleShowOngoing && onToggleShowNotOngoing;
  const isDateFiltered = !showOngoing || !showNotOngoing || filterNoDueDate;
  ...
  const dateFilterCount = [!showOngoing, !showNotOngoing, filterNoDueDate].filter(Boolean).length;
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {departments && departments.length > 0 && onDepartmentChange && (
          <Popover open={deptPopoverOpen} onOpenChange={setDeptPopoverOpen}>
            <PopoverTrigger asChild>
              <Button size="sm" variant={selectedDepartment ? "default" : "outline"} className="h-9 text-xs gap-1 shrink-0 max-w-[200px]">
                <Building2 className="h-3.5 w-3.5 shrink-0" />
                <span className="truncate hidden sm:inline">{selectedDepartment || 'Departamento'}</span>
                <ChevronDown className="h-3 w-3 shrink-0" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-[220px] p-0" align="start">
              <Command>
                ...
                <CommandList>
                  ...
                  <CommandItem
                    onSelect={() => { onDepartmentChange(''); setDeptPopoverOpen(false); }}
                    className="flex items-center gap-2 cursor-pointer"
                  >
                    <span className="flex-1 text-sm">Todos</span>
                    {!selectedDepartment && <Check className="h-3.5 w-3.5 text-primary shrink-0" />}
                  </CommandItem>
                  {departments.map(name => (
                    <CommandItem
                      key={name}
                      onSelect={() => { onDepartmentChange(selectedDepartment === name ? '' : name); setDeptPopoverOpen(false); }}
                      className="flex items-center gap-2 cursor-pointer"
                    >
                      <span className="flex-1 text-sm truncate">{name}</span>
                      {selectedDepartment === name && <Check className="h-3.5 w-3.5 text-primary shrink-0" />}
                    </CommandItem>
                  ))}
                </CommandList>
              </Command>
        ...
        {/* 4. Fecha (fusionado: Continuos + Sin fecha fin) */}
        {(hasDateFilter || onToggleNoDueDate) && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                ...
                {dateFilterCount > 0 && (
                  <Badge variant="secondary" className="text-[9px] h-4 px-1 ml-0.5 bg-primary-foreground/20 text-primary-foreground">{dateFilterCount}</Badge>
                )}
                <ChevronDown className="h-3 w-3" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start">
              ...
              {onToggleNoDueDate && (
                <>
                  {hasDateFilter && <DropdownMenuSeparator />}
                  <DropdownMenuCheckboxItem checked={filterNoDueDate} onCheckedChange={onToggleNoDueDate}>
                    Sin fecha fin
                  </DropdownMenuCheckboxItem>
                </>
              )}
      ...
      {(selectedTags.length > 0 || selectedAssignee || selectedDepartment) && (
